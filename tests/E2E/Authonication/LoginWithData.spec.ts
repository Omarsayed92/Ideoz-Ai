...
import testData from '../../../TestData/testUsers.json';
...
test.describe('Login Page Tests', () => {
    ...
    test('should accept valid email address format', async ({ page }) => {
        ...
        await loginPage.fillEmail(testData.loginCredentials.validUser.email);
        await loginPage.fillPassword(testData.loginCredentials.validUser.password);
    ...
    test('should show error for invalid email address format', async ({ page }) => {
        ...
        await loginPage.fillEmail(testData.loginCredentials.invalidEmailFormats[0]);
    ...
    test('should show error for password that does not meet minimum length', async ({ page }) => {
        ...
        await loginPage.fillEmail(testData.loginCredentials.validUser.email);
        await loginPage.fillPassword(testData.loginCredentials.invalidPasswords[0]);
        await loginPage.clickLogin();

        // Verify validation error appears
        await expect(page.getByText('The password should include 8').or(page.getByText('Field is required'))).toBeVisible();
    });
    ...
    test('should login successfully with valid credentials', async ({ page }) => {
        ...
        await loginPage.login(testData.loginCredentials.validUser.email, testData.loginCredentials.validUser.password);
    ...
    test('should show error message for incorrect credentials', async ({ page }) => {
        ...
        await loginPage.login(testData.loginCredentials.invalidCredentials.email, testData.loginCredentials.invalidCredentials.password);
    ...
    test('should show error message for non-existent account', async ({ page }) => {
        ...
        await loginPage.login(testData.loginCredentials.nonExistentUser.email, testData.loginCredentials.nonExistentUser.password);
    ...
    test('should validate email field on blur', async ({ page }) => {
        ...
        await loginPage.emailField.fill(testData.loginCredentials.invalidEmailFormats[0]);
        await loginPage.passwordField.click(); // Trigger blur

        // Verify inline validation error
        await expect(page.getByText(/invalid email address/i).or(page.locator('.error-message'))).toBeVisible();
    });
    ...
    test('should accept email with special characters in valid format', async ({ page }) => {
        ...
        await loginPage.fillEmail(testData.loginCredentials.validEmailFormats[1]);
        await loginPage.fillPassword('ValidPassword123');
